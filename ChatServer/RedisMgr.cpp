...
RedisConPool::RedisConPool(int pool_size, const char* host, int port, const char* pwd) 
    :_pool_size(pool_size), _host(host), _port(port), _pwd(pwd)
{
...
void RedisConPool::returnConection(redisContext* con)
{
    std::unique_lock<std::mutex> lock(_mtx);
    if (_stop) {
        return;
...
bool RedisMgr::Get(const std::string& key, std::string& value)
{
    ...
    _con_pool->returnConection(connect);
    if (reply == NULL) {
...
bool RedisMgr::LPush(const std::string& key, const std::string& value)
{
    ...
    _con_pool->returnConection(connect);
    if (NULL == reply || reply->type != REDIS_REPLY_INTEGER || reply->integer <= 0)
    {

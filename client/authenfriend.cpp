...
void AuthenFriend::SetApplyInfo(std::shared_ptr<ApplyInfo> apply_info)
{
    ...
    ui->remark_edit->setPlaceholderText(apply_info->_name);
}
...
void AuthenFriend::SlotApplySure()
{
    ...
    QString rmkname = "";
    if(ui->remark_edit->text().isEmpty()){
        rmkname = ui->remark_edit->placeholderText();
    }else{
        rmkname = ui->remark_edit->text();
    }
    jsonObj["rmkname"] = rmkname;
    UserMgr::getInstance()->SetRmkname(_apply_info->_uid, rmkname);

...
struct FriendInfo {
    FriendInfo(int uid, QString name, QString nick, QString icon, int sex, QString desc, QString back, QString last_msg="")
        :_uid(uid), _nick(nick), _name(name), _icon(icon), _sex(sex), _desc(desc), _back(back), _last_msg(last_msg)
    {

    }

    FriendInfo(std::shared_ptr<AuthInfo> auth_info):_uid(auth_info->_uid),
        _nick(auth_info->_nick) ,_name(auth_info->_name), _icon(auth_info->_icon), _sex(auth_info->_sex)
    {

    }

    FriendInfo(std::shared_ptr<AuthRsp> auth_rsp):_uid(auth_rsp->_uid),
        _nick(auth_rsp->_nick), _name(auth_rsp->_name), _icon(auth_rsp->_icon), _sex(auth_rsp->_sex)
    {
...
struct UserInfo {
    ...
    UserInfo(int uid, QString name, QString icon):
        _uid(uid), _name(name), _nick(""), _icon(icon), _sex(0),_last_msg(""){
...
struct TextChatData {
    TextChatData(QString msg_id, QString msg_content, int fromuid, int touid): _msg_id(msg_id), _msg_content(msg_content), _from_uid(fromuid), _to_uid(touid)
    {
